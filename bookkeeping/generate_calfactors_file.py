...
def generate_calfactors_file(datadir,region,wave,goodmaps=False):
    ...
    weighted_file           = np.genfromtxt(sorted(glob.glob('pointsource_results/'+region+'/*'+wave+'*_calepoch_weightedmean'+GMstring+'.txt'))[-1],dtype=None,names=True)
    ...
    datescan_ints           = []
    for eachdatescanstr in weighted_datescans:
        datescan_ints.append(int(str(eachdatescanstr).split('_')[0]))
    datescan_ints = np.array(datescan_ints)
    ...
    output = open('pointsource_results/'+region+'/'+region+'_'+str(datescans_pointing_converted[-1])+'_'+wave+'_CalFactors'+GMstring+'.txt','w') 
    ...
    for ds_point,dxi,dyi in zip(datescans_pointing_converted,dx,dy):
        ...
        output_dx = dxi
        output_dy = dyi
        if ds_point not in weighted_datescans:
            continue
        print('weighted_datescans,ds_point')
        print(weighted_datescans,ds_point)
        
        Weighted_index = np.where(weighted_datescans == ds_point)
        if len(Weighted_index[0])<1:
            output_weighted = 'nan'
        ...
        print('datescans_ACcal_converted,ds_point')
        print(datescans_ACcal_converted,ds_point)
        ACcal_index = np.where(datescans_ACcal_converted==ds_point)
        if len(ACcals[ACcal_index])<1:
            output_ac = 'nan'
        else:
            print('ACcals,ACcal_index,ACcals[ACcal_index],weighted_calfactors,weighted_index,weighted_calfactors[weighted_index]')
            print(ACcals)
            print('\n',ACcal_index)
            print('\n',ACcals[ACcal_index])
            print('\n',weighted_calfactors)
            print('\n',Weighted_index)
            print('\n',weighted_calfactors[Weighted_index])
            output_ac = round(ACcals[ACcal_index][0]/output_weighted,3)

        print('PS_datescans,ds_point')
        print(PS_datescans,ds_point)
        PScal_index = np.where(PS_datescans==ds_point)
        if len(PScal_index[0])<1:
            output_ps = 'nan'
            output_psunc = 'nan'
        else:
